...
export default async function verifyUser(email: string, registerMode = false): Promise<any> {
  try {
    if (!registerMode) {
      const perm_group = await getPermissionGroup()

      if (!perm_group) {
        return handleError(new Error("Unauthorized to verify user"))
      }
    }
    ...
    if (!response.ok) {
      const unhandledError = new Error(data?.message || "Failed to verify user")
