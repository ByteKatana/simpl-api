...
export async function getServerSideProps() {
	...
	let resNamespace = await axios.get(
		`${process.env.BASE_URL}/api/v1/entry-types?apikey=${process.env.API_KEY}`
	)
...
export default function Settings({ fetchedPermissionGroups, fetchedNamespaces, fetchedApiKeys }) {
	...
	useEffect(() => {
		...
		function calcPermissionGroupPages() {
			...
			for (
				let i = 1;
				i <= fetchedPermissionGroups.length / paginationPermissionGroupState.limit + 1;
				i++
			) {
				count.push(i)
			}
		...
		const getActivePermissions = () => {
			...
			fetchedPermissionGroups[permGroupIndex].privileges.map((privilege, index) => {
	...
	const checkPermission = (permissionName: string) => {
		...
		fetchedPermissionGroups[permGroupIndex].privileges.map((privilege, index) => {
			...
			if (privilege[namespaceKey] !== undefined) {
				let activePerms = privilege[namespaceKey].permissions
				if (activePerms.includes(permissionName)) result = true
			}
		})
									...
									<div className="w-4/12">
										...
										<ul className="mt-3 cursor-pointer justify-start">
											{fetchedPermissionGroups
												.slice(
													paginationPermissionGroupState.min,
													paginationPermissionGroupState.max
												)
									...
									<div className="w-4/12 px-5">
										...
										<div className="flex justify-start mt-3">
											<div>
												...
												<div
													id="send_permission_btn"
													className="flex flex-row flex-nowrap justify-center  mt-5">
													<div className=" w-11/12">
														<button
