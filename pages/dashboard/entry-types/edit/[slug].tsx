...
export async function getServerSideProps(req: NextResponse) {
	...
	const resEntryTypes = await axios.get(
	...
	entryType[0].fields.map((field, index) => {
		let fieldKey = Object.keys(field)
...
export default function EditEntryType({
	entryTypesData,
	entryTypeData,
	fieldsData,
	fetchedPermGroups
	...
	const submitData = async () => {
		...
		if (Object.keys(formErrors).length > 0) {
			...
			if (
				...
				entryType[0].namespace === entryType[0].name.toLowerCase()
			) {
				formatedEntryType = {
					name: entryType[0].name,
