...
export default function CreateEntryType({ fetchedEntryTypes }) {
	const [entryType, setEntryType] = useState({ name: "", slug: "", namespace: "itself" })
	const [formFields, setFormFields] = useState([
		{ field_name: "", field_value_type: "", field_form_type: "", field_length: 100 }
	])
		...
		if (event.target.name === "name" || event.target.name === "namespace") {
			...
			if (event.target.value === "") {
				...
				if (`${event.target.name}_${index}` in formErrors) {
					let copyErrors = { ...formErrors }
					const { [`${event.target.name}_${index}`]: undefined, ...restOfErrors } = copyErrors
					setFormErrors(restOfErrors)
	...
	const submitData = async () => {
		if (Object.keys(formErrors).length > 0) {
			...
			if (
				...
				entryType.namespace === entryType.name.toLowerCase()
			) {
				...
				resultSwal
					...
					.then((result) => {
						if (result.isConfirmed) {
							...
							document.getElementById("entry_type_create_form").reset()
								...
								{formFields.map((field, index) => {
									return (
										<div
											...
											<div
												key={`field_block_${index}`}
												className="flex flex-col justify-center block w-11/12">
												<div>
													<label
